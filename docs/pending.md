...
- Task 6 (admin usability): the "?" help links themselves were never click-tested end-to-end before this task wired up the matching `docs/user-guide/admin-guide.md` anchors; `admin/js/sections/log.js`'s audit-action label falls back to the raw action string for any future action not yet added to `admin.log.action.*` (by design, not a bug — see that file's own comment). Final-review fix wave M8: `admin/js/sections/log.js`'s 📜 লগ card's অডিট/ত্রুটি tabs (`tabAudit`/`tabErrors`) are plain `<button>`s with a manually toggled `.active` class, not a real ARIA `role="tab"`/`role="tabpanel"`/arrow-key pattern (unlike events.js's day tabs, which do have real tab ARIA per item 21) — deferred, not fixed in this wave (out of its file list); revisit if this card's accessibility gets a dedicated pass.
