  ...
  for (const theme of THEMES) {
    ...
    for (const w of WIDTHS) {
      ...
      for (const p of PAGES) {
        try {
          // 'networkidle' never resolves on this site: the shell's live-announcements ticker keeps
          // a persistent Firestore connection open on every page. 'load' + a settle pause is what
          // the existing e2e specs in this repo rely on too (none of them use 'networkidle').
          await page.goto(`${base}/${p}.html?theme=${theme}`, { waitUntil: 'load' });
          await page.waitForTimeout(600);
          const overflow = await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
          if (overflow > 0) { console.warn(`OVERFLOW ${theme}/${p}@${w}: ${overflow}px`); process.exitCode = 1; }
          await page.screenshot({ path: `test-results/shots/${theme}/${p}-${w}.png`, fullPage: true });
        } catch (err) {
          console.warn(`FAIL ${theme}/${p}@${w}: ${err.message}`);
          process.exitCode = 1;
        }
      }
      await page.close();
