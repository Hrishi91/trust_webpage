...
registerSection(COLL, {
  title: STRINGS['admin.culture'], icon: '🏺',
  async render(box, ctx) {
    ...
    const f = {
      title: biField(STRINGS['admin.culture.title'], 'title', cur.title),
      tag: biField(STRINGS['admin.culture.tag'], 'tag', cur.tag),
      text: biField(STRINGS['admin.culture.text'], 'text', cur.text, { multiline: true }),
      image: imageField(ctx, STRINGS['admin.culture.image'], cur.imageUrl, { folder: 'public/culture', max: 1200 }),
    };
